...
/// @title Peach - V14-15 is a gas-usage test of using bytes32 instead of string for _names
    ...
    UUPSUpgradeable
{
    ...
    function setToken(
        ...
        _setToken(tokenId, bytes(name));
    }

    // DONE: consider moving all the _modName() checks into modName(), then inline _modName()
    function _setToken(uint tokenId, bytes memory name) internal {
        _mint(msg.sender, tokenId); // creates token (first ensures token doesn't exist)
        // _modName(tokenId, name); // names token
        _names[tokenId] = bytes32(name); // rename token (first ensures token is owned, which also ensures that it exists)
        emit Rename("", bytes32(name), tokenId);
    ...
    function modName(string calldata colorhex, string memory newName) external {
        uint tokenId = aGetId(colorhex); // gets tokenId
        if (bytes(newName).length > 32) revert InvalidTokenName();
        _modName(tokenId, bytes(newName));
    }
    ...
    function _modName(
        ...
        _names[tokenId] = bytes32(newName); // rename token (first ensures token is owned, which also ensures that it exists)
        emit Rename(oldName, bytes32(newName), tokenId);
    }
